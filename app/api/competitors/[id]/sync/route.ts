...
const APIFY_ACTOR = 'AQ1yCqZf7u2s4Ig0j'
...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  ...
  const searchUrl = `https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=ALL&media_type=all&q=${encodeURIComponent(competitor.name)}&search_type=keyword_unordered`

  const res = await fetch(`https://api.apify.com/v2/acts/${APIFY_ACTOR}/runs?token=${token}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ startUrls: [{ url: searchUrl }], totalRecords: 50 }),
  })
...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  const items: { result?: Record<string, unknown>; success?: boolean }[] = await resultsRes.json()

  const ads = items.filter(item => item.result && !(item.result as { error?: string }).error)
  ...
  const rows = ads.map(ad => {
    ...
    return {
      ...
      ad_archive_id: String(ad.ad_archive_id ?? Math.random()),
