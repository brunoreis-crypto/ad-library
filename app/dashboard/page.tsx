    ...
    <AppShell>
      <div className="px-8 py-7">
        ...
        <div className="mb-6">
          ...
          <p className="text-slate-500 text-sm mt-1">Visão geral de todos os clientes ativos</p>
        </div>

        {/* Metric cards */}
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
          <MetricCard label="Clientes ativos" value={String(m.active_clients)} icon={Users} color="indigo" />
          ...
          <MetricCard label="Impressões" value={fmt(m.total_impressions)} icon={Eye} color="slate" />
          <MetricCard label="CTR médio" value={`${m.avg_ctr.toFixed(2)}%`} icon={MousePointer} color="indigo" />
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-slate-800">Top anúncios por CPL</h2>
            <a href="/clients" className="text-indigo-600 text-sm hover:underline">Ver todos →</a>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {topAds.map(ad => (
              <AdCard key={ad.id} ad={ad} showClient />
            ))}
          </div>
        </div>
      </div>
